...
void Model::render()
{
	...
	//point to vertex, normal and texture arrays
...
bool Model::loadModel(char* filename)
{
	...
	//load first 3 values into vertex, texture and normal coordinates then increment by 3 until all faces are loaded
	// the -1 simbolises the fact that the array starts at 0 not 1
	for (int i = 0; i < faces.size(); i = i + 3)
	{
		//push back first value in the array (vertex)
		vertex.push_back(verts[faces[i] - 1].x);
		vertex.push_back(verts[faces[i] - 1].y);
		vertex.push_back(verts[faces[i] - 1].z);

		//push back second value in the array (texture coordinates)
		texCoords.push_back(texCs[faces[i + 1] - 1].x);
		texCoords.push_back(texCs[faces[i + 1] - 1].y);

		//push back third value in the array (normal)
